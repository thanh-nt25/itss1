...
const ReviewTest = ({ test }) => {
  ...
  const handleDownloadAnswers = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text(`テスト名: ${test.title}`, 10, 10);
        ...
        <div className="w-3/4 bg-white p-4 rounded shadow">
          ...
          <div className="mb-4">
            <p className="text-gray-800 mb-2">
              <span className="font-bold">答え:</span> {test.questions[currentQuestion].answer}
            </p>
            <p className={`mb-4 font-bold ${test.questions[currentQuestion].isCorrect ? "text-green-600" : "text-red-600"}`}>
              {test.questions[currentQuestion].isCorrect ? "正しい" : "間違い"}
            </p>
            <p className="text-gray-600">
              <span className="font-bold">説明:</span> {test.questions[currentQuestion].explanation || "説明がありません。"}
            </p>
          </div>
        </div>
      </div>
